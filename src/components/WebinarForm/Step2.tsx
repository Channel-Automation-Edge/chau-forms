...
const Step2: React.FC<Step2Props> = ({ onNext }) => {
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    try {
      ...
      const sheetResponse = await fetch(import.meta.env.VITE_GOOGLE_WEBINAR_API, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
              ...
              <div className='space-y-6 px-4 md:pl-0 text-center md:text-left w-full max-w-2xl'>
                <div className='space-y-2'>
                  ...
                  <p className='text-base md:text-lg text-gray-800 md:pr-8'>
                    Sign up for our webinar and get exclusive insights!
                  </p>
                </div>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader className='items-center'>
            ...
            <DialogDescription className='text-center'>
              You have successfully signed up for our webinar. We look forward to seeing you there!
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
